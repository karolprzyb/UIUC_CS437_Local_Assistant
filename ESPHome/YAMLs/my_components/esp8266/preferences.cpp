  ...
  ESPPreferenceObject make_preference(size_t length, uint32_t type, bool in_flash) override {
    ...
    if (in_flash) {
      ...
      // should never get to this because ESPHome only saves a pretty small number of well defined things.
        ...
        if ( this->current_flash_offset < 66 ) {this->current_flash_offset = 66;}
      ...
      ESP_LOGV(TAG, "Making Preference in Flash - start: %u: length: %u, length_words:%u type: %u", start, length, length_words, type);

...
void LightState::loop() {
  ...
  if (this->transformer_ != nullptr) {
    ...
    if (this->transformer_->is_finished()) {
      ...
      // this may need to actually be added into ESPHome because for 15s fade with no return its not
      // writing the final target state.  We don't even need this anymore since we return a value now.
      this->next_write_ = true;
